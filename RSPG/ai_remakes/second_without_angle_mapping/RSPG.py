...
class AI():
    def __init__(self,all_planes,all_bullets,all_xp,NW_OW="server"):
        ...
        self.points = []
        self.sec_tick_ops()
        self.vew_dist = random.randint(650,800)
        self.comp_biost = random.random()
        self.T_threashuld = random.random()
    ...
    def find_closest_obj(self, all_objects, distance):
        closest = None
    ...
    def calculate_controls(self):
        ...
        decided = False
        if self.target == None:
            T = self.find_closest_obj(self.all_planes,self.vew_dist)
            P = self.find_closest_obj(all_xp,self.vew_dist)
            T_Dval = self.whay_ops(T)
            P_Dval = self.whay_ops(P)
            if T_Dval >= self.T_threashuld:
                self.target = T
            elif P_Dval > self.T_threashuld:
                self.target = P
            else:
                self.target = T
            
        if self.target != None:
            if self.target.value <= self.T_threashuld:
                leftRite = self.run(self.target)
                frontBack = 1
            elif self.target.value > self.T_threashuld:
                try:
                    pt = self.target.PT
